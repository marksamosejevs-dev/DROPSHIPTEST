...
const f = (v: Field, lang: Locale) =>
  isPlaceholder(v) ? `<span class="ph" title="${esc(v.note)}" data-placeholder>[${tbc[lang]}]</span>` : esc(v);
...
function controller(c: Ctx) {
  ...
  return `<table><tbody>${rows.map((r, i) => `<tr><th scope="row">${r}</th><td>${vals[i]}</td></tr>`).join('')}</tbody></table>`;
}
    ...
    lv: {
      privacy: {
        ...
        sections: [
          ...
          { h: '6. Glabāšanas termiņi', html: `<ul><li>Pieteikumi, kas nav noveduši pie līguma: līdz <span class="ph">[12 mēnešiem — apstiprināt]</span> pēc pēdējās saziņas.</li><li>Līguma un grāmatvedības dokumenti: tik ilgi, cik to nosaka Latvijas normatīvie akti, un garantijas perioda laikā.</li><li>Piekrišanas ieraksti: kamēr piekrišana ir spēkā un pēc tam tik ilgi, cik nepieciešams tās pierādīšanai.</li></ul>` },
          { h: '7. Jūsu tiesības', html: `<p>Jums ir tiesības piekļūt saviem datiem, labot tos, dzēst tos, ierobežot apstrādi, iebilst pret apstrādi, kas balstīta uz leģitīmām interesēm, un saņemt datus pārnesamā formātā. Ja apstrāde balstīta uz piekrišanu, jūs varat to jebkurā laikā atsaukt — sīkdatnēm to var izdarīt ${settings}sīkdatņu iestatījumos</a>. Pieprasījumus sūtiet uz ${f(company.privacyEmail, lang)}.</p><p>Jums ir tiesības iesniegt sūdzību Datu valsts inspekcijai (<a href="https://www.dvi.gov.lv" rel="noopener" target="_blank">www.dvi.gov.lv</a>).</p>` },
      ...
      terms: {
        ...
        sections: [
          ...
          { h: '8. Kontakti', html: `<p>${L}, ${f(company.registeredAddress, lang)}, ${f(company.email, lang)}.</p>` },
        ],
      },
      legal: {
        ...
        sections: [
          { h: 'Vietnes pārvaldītājs', html: controller(c) + `<p>PVN maksātāja numurs: ${f(company.vatNumber, lang)}</p>` },
          { h: `Saistība ar ${esc(group.name)}`, html: `<p>${f(group.relationship, lang)}</p><p><a href="${group.url}" rel="noopener" target="_blank">${esc(group.url)}</a></p>` },
    ...
    ru: {
      privacy: {
        ...
        sections: [
          ...
          { h: '6. Сроки хранения', html: `<ul><li>Заявки, не приведшие к договору: до <span class="ph">[12 месяцев — подтвердить]</span> после последнего контакта.</li><li>Договорные и бухгалтерские документы: в течение срока, установленного законодательством Латвии, и в течение гарантийного периода.</li><li>Записи о согласии: пока согласие действует и затем столько, сколько необходимо для его подтверждения.</li></ul>` },
          { h: '7. Ваши права', html: `<p>Вы вправе получить доступ к своим данным, исправить их, удалить, ограничить обработку, возразить против обработки на основании законных интересов и получить данные в переносимом формате. Если обработка основана на согласии, вы можете отозвать его в любое время — для cookie это можно сделать в ${settings}настройках cookie</a>. Запросы направляйте по адресу ${f(company.privacyEmail, lang)}.</p><p>Вы вправе подать жалобу в Государственную инспекцию данных Латвии (<a href="https://www.dvi.gov.lv" rel="noopener" target="_blank">www.dvi.gov.lv</a>).</p>` },
      ...
      terms: {
        ...
        sections: [
          ...
          { h: '8. Контакты', html: `<p>${L}, ${f(company.registeredAddress, lang)}, ${f(company.email, lang)}.</p>` },
        ],
      },
      legal: {
        ...
        sections: [
          { h: 'Оператор сайта', html: controller(c) + `<p>Номер плательщика НДС: ${f(company.vatNumber, lang)}</p>` },
          { h: `Связь с ${esc(group.name)}`, html: `<p>${f(group.relationship, lang)}</p><p><a href="${group.url}" rel="noopener" target="_blank">${esc(group.url)}</a></p>` },
    ...
    en: {
      privacy: {
        ...
        sections: [
          ...
          { h: '6. Retention', html: `<ul><li>Requests that do not lead to a contract: up to <span class="ph">[12 months — confirm]</span> after the last contact.</li><li>Contract and accounting records: for the period required by Latvian law and throughout the warranty period.</li><li>Consent records: while the consent is valid and afterwards for as long as needed to demonstrate it.</li></ul>` },
          { h: '7. Your rights', html: `<p>You have the right to access, rectify and erase your data, to restrict processing, to object to processing based on legitimate interests and to data portability. Where processing is based on consent, you may withdraw it at any time — for cookies, via the ${settings}cookie settings</a>. Send requests to ${f(company.privacyEmail, lang)}.</p><p>You have the right to lodge a complaint with the Data State Inspectorate of Latvia (<a href="https://www.dvi.gov.lv" rel="noopener" target="_blank">www.dvi.gov.lv</a>).</p>` },
      ...
      terms: {
        ...
        sections: [
          ...
          { h: '8. Contact', html: `<p>${L}, ${f(company.registeredAddress, lang)}, ${f(company.email, lang)}.</p>` },
        ],
      },
      legal: {
        ...
        sections: [
          { h: 'Website operator', html: controller(c) + `<p>VAT number: ${f(company.vatNumber, lang)}</p>` },
          { h: `Relationship with ${esc(group.name)}`, html: `<p>${f(group.relationship, lang)}</p><p><a href="${group.url}" rel="noopener" target="_blank">${esc(group.url)}</a></p>` },
